...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.phone_number_field = self.UserModel._meta.get_field('phone_number')
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help='Specifies the login field username',
        )

        parser.add_argument(
            '--email',
            dest='email', default=None,
            help='Specifies the login field email',
        )

        parser.add_argument(
            '--phone_number',
            dest='phone_number', default=None,
            help='Specifies the login field phone number',
        )
        ...
        if not options['interactive']:
            try:
                if not username or email or phone_number:
                    raise CommandError("You must use --username or --email or --phone_number with --noinput." % self.UserModel.USERNAME_FIELD)
            ...
            try:
                ...
                self.validate_data(field_name='email', field_value=email, database=database)
                self.validate_data(field_name='phone_number', field_value=phone_number, database=database)

                # Validate data inputted by --< field_name >
                for field_name in self.UserModel.REQUIRED_FIELDS:
                    self.validate_data(field_name=field_name, field_value=options[field_name])
                ...
                while email is None:
                    ...
                    if self.email_field.unique:
                        try:
                            self.UserModel._default_manager.db_manager(database).get(email=email)
                        except self.UserModel.DoesNotExist:
                            pass
                        else:
                ...
                # Get a email
                verbose_field_name = self.phone_number_field.verbose_name
                while phone_number is None:
                    input_msg = '%s: ' % capfirst(verbose_field_name)
                    phone_number = self.get_input_data(self.phone_number_field, input_msg)

                    if not phone_number:
                        break
                    if self.phone_number_field.unique:
                        try:
                            self.UserModel._default_manager.db_manager(database).get(phone_number=phone_number)
                        except self.UserModel.DoesNotExist:
                            pass
                        else:
